# description : llvm lang compiles to js, python, c

# docs
https://nim-lang.org
https://telatin.github.io/nim-for-bioinformatics/docs/common-tasks/args/
...
# nim
https://gist.github.com/sdwfrost/7c660322c6c33961297a826df4cbc30d # compare to julia
nim c --cc:gcc --gcc.exe=gcc-6 -d:release --passC="-flto -ffast-math" sir # compile using gcc
nim c -d:release -d:wasm -o:sir_opt.js  sir_opt  # compile to js with emscripten
<nimcmd> --passC:-fopenmp --passL:-fopenmp  # compile with openmp
<nimcmd> --passC:=-flto # floating point optimizations
<nimcmd> -d:release # remove debugging / tracing code
<nimcmd> --passC="-ffast-math -flto" # fast math

# create nim bindings for rust libs: nbindgen
...
https://telatin.github.io/nim-for-bioinformatics/docs/common-tasks/args/
see docopt or cligen

# C interop
https://github.com/PMunch/futhark -- C header to nim wrapper
c2nim --
nimterop -- treesitter parser to nim

# testing -- testament (builtin)
testament run tests/category/test0.nim
testament r tests/category/test0

# autosplit (like perl -ane)
# ...
